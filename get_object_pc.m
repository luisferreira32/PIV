function [pc]=get_object_pc(pixel_list, imgrgb,imgdepth)
% Inputs: 
...
for i=1:length(pixel_list)
    rgb_index(i,:)=imgrgb(pixel_list(i,2),pixel_list(i,1),:);
end

% Get the pixel values from the depth image 

for i=1:length(pixel_list)
    depth_index(i)=imgdepth(pixel_list(i,2),pixel_list(i,1));
end
