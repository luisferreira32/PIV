...
for i=1:length(d1)
    ...
    n_matches(i) = length(matches);
end
[~, mm_index] = sort(n_matches);

P1=[];
P2=[];
% % use the 10% better
for i=1:mm_index(1:fix(length(mm_index)*0.1))
    I1=single(rgb2gray(uint8(rgbd1(:,:,:,i))));
    I2=single(rgb2gray(uint8(rgbd2(:,:,:,i))));
    [f1,d1_]=vl_sift(I1);
    [f2,d2_]=vl_sift(I2);
...
Tfab = [1 0 1];
xyz2fab1 = xyz1(:,:,1)*Rfab + ones(length(xyz1(:,:,1)),1)*Tfab;
...
for i=1:(fix(length(xyz2fab1)/1000))
    p1 = xyz1(i*900,:,1) %+ [rand()*0.2 rand()*0.2 rand()*0.2];
    p2 = xyz2fab1(i*900,:) %+ [rand()*0.2 rand()*0.2 rand()*0.2];
    P1 = [P1; p1];
    P2 = [P2; p2];
end
final_inds=find((P1(:,3).*P2(:,3))>0);
